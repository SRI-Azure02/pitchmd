...
export class SnowflakeClient {
  ...
  // ─── Phase 4: RAG Document Methods ───────────────────────────────────────

  /** Register an approved document in SYNTHETIC_COMPLIANCE_DOCUMENTS. */
  async registerDocument(params: {
    ...
    const rows = await this.executeQuery(`
      INSERT INTO CORTEX_TESTING.PUBLIC.SYNTHETIC_COMPLIANCE_DOCUMENTS
        (DOC_NAME, DOC_TYPE, PRODUCT, MLR_STATUS, APPROVED_BY, APPROVED_DATE)
      VALUES (:1, :2, :3, 'approved', :4, CURRENT_DATE())
    `, {
      '1': { type: 'TEXT', value: params.docName },
      '2': { type: 'TEXT', value: params.docType },
      '3': { type: 'TEXT', value: params.product ?? '' },
      '4': { type: 'TEXT', value: params.approvedBy },
    });
    // Fetch the newly created DOC_ID
    const idRows = await this.executeQuery(`
      SELECT DOC_ID FROM CORTEX_TESTING.PUBLIC.SYNTHETIC_COMPLIANCE_DOCUMENTS
      WHERE DOC_NAME = :1 AND APPROVED_BY = :2
      ORDER BY CREATED_AT DESC LIMIT 1
    `, {
      '1': { type: 'TEXT', value: params.docName },
      '2': { type: 'TEXT', value: params.approvedBy },
    });
    return (idRows as any[])[0]?.DOC_ID ?? '';
  }

  /**
   * Insert a document chunk with vector embedding generated by Snowflake Cortex.
   * Caller specifies which 1024-dim model to use (probed in the ingest route).
   */
  async ingestDocumentChunk(params: {
    docId: string;
    chunkText: string;
    chunkIndex: number;
    ...
    // Model name is interpolated (not bound) because Snowflake binding
    // does not support string literals inside function names.
    const safeModel = params.model.replace(/[^a-zA-Z0-9\-_.]/g, '');
    await this.executeQuery(`
      INSERT INTO CORTEX_TESTING.PUBLIC.SYNTHETIC_DOCUMENT_CHUNKS
    ...
    `, {
      ...
      '3': { type: 'TEXT', value: String(params.chunkIndex) },
      '4': { type: 'TEXT', value: params.pageNumber !== null ? String(params.pageNumber) : '' },
      '5': { type: 'TEXT', value: params.sectionLabel ?? '' },
    });
  }
  ...
  async ingestDocumentChunkNoEmbedding(params: {
    ...
    await this.executeQuery(`
      INSERT INTO CORTEX_TESTING.PUBLIC.SYNTHETIC_DOCUMENT_CHUNKS
        (DOC_ID, CHUNK_TEXT, CHUNK_INDEX, PAGE_NUMBER, SECTION_LABEL)
      VALUES (:1, :2, :3, :4, :5)
    `, {
      '1': { type: 'TEXT', value: params.docId },
      '2': { type: 'TEXT', value: params.chunkText },
  ...
  /**
   * Retrieve relevant chunks using vector similarity (preferred) or keyword
   * matching (fallback when chunks have no embeddings).
   *
   * Vector path: uses VECTOR_COSINE_SIMILARITY with Cortex query embedding.
   * Keyword path: uses ILIKE on key terms from the query text.
   */
  async searchSimilarChunks(queryText: string, limit = 5): Promise<any[]> {
    const safeLimit = Math.max(1, Math.min(20, Math.floor(limit)));

    // Check whether any chunks have embeddings
    const hasEmbeddings = await this.executeQuery(`
      SELECT COUNT(*) AS CNT
      FROM CORTEX_TESTING.PUBLIC.SYNTHETIC_DOCUMENT_CHUNKS
      WHERE EMBEDDING IS NOT NULL
      LIMIT 1
    `, {});
    const embeddingCount = Number((hasEmbeddings as any[])[0]?.CNT ?? 0);
    ...
    // ── Keyword fallback ──────────────────────────────────────────────────
    // Extract meaningful terms (≥4 chars, skip stop words)
    const stopWords = new Set(['with','that','this','from','have','been','will','they','what','when','your','which']);
  ...
  async getComplianceDocuments(): Promise<any[]> {
    return await this.executeQuery(`
      SELECT
        d.DOC_ID, d.DOC_NAME, d.DOC_TYPE, d.PRODUCT,
        d.MLR_STATUS, d.APPROVED_BY, d.APPROVED_DATE,
        COUNT(c.CHUNK_ID) AS CHUNK_COUNT
      FROM CORTEX_TESTING.PUBLIC.SYNTHETIC_COMPLIANCE_DOCUMENTS d
      LEFT JOIN CORTEX_TESTING.PUBLIC.SYNTHETIC_DOCUMENT_CHUNKS c
        ON d.DOC_ID = c.DOC_ID
      GROUP BY d.DOC_ID, d.DOC_NAME, d.DOC_TYPE, d.PRODUCT,
               d.MLR_STATUS, d.APPROVED_BY, d.APPROVED_DATE

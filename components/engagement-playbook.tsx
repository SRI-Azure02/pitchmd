  ...
  const renderPlaybookContent = (physicianId: string) => {
    ...
    return (
      <div className="space-y-3">
        ...
        {/* ── Row 1: Rx activity + Recent Interactions side by side ──────── */}
        <div className="grid grid-cols-2 gap-3" style={{ alignItems: 'stretch' }}>

          {/* Left: physician brief + market share */}
          ...
          <div className="rounded-xl border border-slate-200 bg-white overflow-hidden flex flex-col">
            <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50/60 flex items-center gap-2 shrink-0">
            ...
            {activity.length === 0 ? (
              <div className="px-4 py-3 flex-1 flex items-center">
                <p className="text-sm text-slate-400">No recent activity on record.</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100 flex-1 overflow-auto">
                {activity.map((a, i) => (
                  <div key={i} className="px-4 py-2.5 flex items-start gap-3">
                    <span className="mt-[7px] w-1.5 h-1.5 rounded-full bg-sky-400 shrink-0" />
